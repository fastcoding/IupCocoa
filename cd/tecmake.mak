...
ifdef USE_LUA51
  LIBLUA_SFX := 51
  ifdef USE_PKGCONFIG
  PKGINCS += $(shell pkg-config --cflags lua-5.1)
  PKGLIBS += $(shell pkg-config --libs lua-5.1)
  endif
endif
ifdef USE_LUA52
  LIBLUA_SFX := 52
...
ifdef USE_GTK
  ...
  ifdef USE_PKGCONFIG
    ...
    PKGINCS += $(shell pkg-config --cflags gtk+-$(GTKSFX).0 gtk+-unix-print-$(GTKSFX).0 gdk-$(GTKSFX).0)
    PKGLIBS += $(shell pkg-config --libs gtk+-$(GTKSFX).0 gtk+-unix-print-$(GTKSFX).0 gdk-$(GTKSFX).0)
